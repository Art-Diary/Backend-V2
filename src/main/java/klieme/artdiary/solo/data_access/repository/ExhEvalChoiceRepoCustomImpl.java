...
public class ExhEvalChoiceRepoCustomImpl implements ExhEvalChoiceRepoCustom {
	private final JPAQueryFactory query;

	@Override
	public List<Map<String, Object>> getChoices(Long exhId, Long userId) {
		...
		List<Tuple> tuples = query
			...
			.from(exhEvalChoice)
			.leftJoin(evalOption).on(exhEvalChoice.exhEvalChoiceId.optionId.eq(evalOption.optionId))
			.leftJoin(evalFactor).on(evalOption.factorId.eq(evalFactor.factorId))
			.fetchJoin()
			.where(exhEvalChoice.exhEvalChoiceId.exhId.eq(exhId),
				exhEvalChoice.exhEvalChoiceId.userId.eq(userId))
